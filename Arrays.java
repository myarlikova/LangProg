...
class Arrays {
	...
	for(int i = 0; i < 5; i++) 
		...
		for(int k = 0; k < 2; k++)
			riders[i][k] = i+ k + 10;
	//Вывод массива
	System.out.println("перевкезено пассажиров в будние дни: ");
	for(int i = 0; i < 5; i++) {
                for(int k = 0; k < 10; k++)
	...
	System.out.println();
	System.out.println("перевeзено пассажиров в выходные дни: ");
        for(int i = 5; i < 7; i++) {
                for(int k = 0; k < 2; k++)
                       System.out.println(riders[i][k] + " ");
		System.out.println();
	...
	//Альтернативный вариант int nums[]  = new nums [];
	...
	System.out.println("Массив nums3: ");
        for(int i = 0; i < 10; i++) {
	...
	System.out.println("Длина table " + table1.length);
